...
class MyPatient extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: Scaffold(
          appBar: appBar('My patients', center: false, action: []),
    ...
    return StreamBuilder<List<Patient>>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          return const Center(
              child: Text('there is something is wrong with the connection'));
          ...
          final data = snapshot.data!
              .where((element) => element.operatName == uid)
              .toList();
          return Column(
            children: [
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, i) {
                    return PatientContainer(
                      ...
                      userId: data[i].uid, state: 'Dismiss?',
                      onpress: () async {
